...
class Logger:
    ...
    def _base_logger():
        ...
        threading.Thread(target=Logger._event_loop.run_forever, daemon=True).start()
    ...
    def _flush_logs():
        ...
        asyncio.run_coroutine_threadsafe(
            _await_logger_complete(), Logger._event_loop
        ).result()
        Logger._event_loop.call_soon_threadsafe(Logger._event_loop.stop)
    ...
    def add_sink(sink, *args, **kwargs) -> int:
        ...
        if inspect.iscoroutinefunction(sink):
            kwargs["loop"] = Logger._event_loop
